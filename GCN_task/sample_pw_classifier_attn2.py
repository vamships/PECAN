...
class PWClassifier(object):
    def __init__(self, layer_specs, layer_args, train_data, learning_rate, pn_ratio, outdir):
        ...
        with self.graph.as_default():
            ...
            self.subsample_r = tf.placeholder(tf.int32,[None], "subsample_r")
            ...
            input2 = self.output_conv_p2, self.in_edge2, self.in_hood_indices2

            name = "leg1_{}_{}".format(type, i)
            #print("Leg1: ",name)
            with tf.name_scope(name):
                # initialize layer with parameters
                self.output_conv2, params = layer_fn(input1, None, **args)
                if params is not None:
                    self.params.update({"{}_{}".format(name, k): v for k, v in params.items()})

                self.output_conv2_p2, _ = layer_fn(input2, params, **args)

                self.wt_conv2_center = params["Wc"]
                self.wt_conv2_neighb = params["Wn"]
                self.bias_conv2 = params["b"]
            ...
            name = "leg1_attention"
            self.sel_vertex1 = tf.gather(self.output_conv2, self.examples)
            
            input_att_1 = self.output_conv2, self.output_conv2_p2, self.pairs
            with tf.name_scope(name):
    ...
    def weights(self, data):
        output_list = self.run_graph([self.wt_conv_center, self.wt_conv_neighb, self.bias_conv,
                                        self.wt_conv2_center, self.wt_conv2_neighb, self.bias_conv2,
                                        self.wt_attention,
        ...
        return [{"wt_conv_center":output_list[0], "wt_conv_neighb":output_list[1], "bias_conv":output_list[2]},
                {"wt_conv_center":output_list[3], "wt_conv_neighb":output_list[4], "bias_conv":output_list[5]},
                {"wt_attention":output_list[6]},
                {"wt_dense":output_list[7], "bias_dense":output_list[8]}]
    ...
    def activate(self, data): 
        ...
        return [{"output_conv":output_list[0]},
                ...
                {"feats_attention":output_list[2],"raw_attention":output_list[3], "output_attention":output_list[4]},
                {"output_dense":output_list[5], "output_preds":output_list[6], "labels":output_list[7]}]
