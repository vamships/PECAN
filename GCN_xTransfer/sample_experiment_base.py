...
from sample_pw_classifier_base import PWClassifier
...
wt_dense = np.genfromtxt(dir_preTrain+"0.ann/wt_dense.csv",dtype=np.float32,delimiter=",")
wt_dense = np.expand_dims(wt_dense,axis=1)
bs_dense = np.array([np.genfromtxt(dir_preTrain+"0.ann/bias_dense.csv",dtype=np.float32,delimiter=",")])

print(wt_dense.shape,bs_dense.shape)
params_preTrain = {"W": wt_dense, "b": bs_dense}
