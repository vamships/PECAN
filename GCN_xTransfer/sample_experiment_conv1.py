...
wt_dense = np.genfromtxt(dir_preTrain+"1.ann/wt_dense.csv",dtype=np.float32,delimiter=",")
wt_dense = np.expand_dims(wt_dense,axis=1)
bs_dense = np.array([np.genfromtxt(dir_preTrain+"1.ann/bias_dense.csv",dtype=np.float32,delimiter=",")])
params_dense = {"W": wt_dense, "b": bs_dense}

print(wt_conv_center.shape,wt_conv_neighb.shape, bs_conv.shape)
params_preTrain = [params_conv, params_dense]
