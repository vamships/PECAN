...
from sample_pw_classifier_attn1 import PWClassifier
...
wt_attn = np.genfromtxt(dir_preTrain+"1.attend_on_other/wt_attention.csv",dtype=np.float32,delimiter=",")
params_attn = {"Wc":wt_attn}

wt_dense = np.genfromtxt(dir_preTrain+"2.ann/wt_dense.csv",dtype=np.float32,delimiter=",")
wt_dense = np.expand_dims(wt_dense,axis=1)
bs_dense = np.array([np.genfromtxt(dir_preTrain+"2.ann/bias_dense.csv",dtype=np.float32,delimiter=",")])
params_dense = {"W": wt_dense, "b": bs_dense}

print(wt_conv_center.shape,wt_conv_neighb.shape, bs_conv.shape)
params_preTrain = [params_conv, params_attn, params_dense]
